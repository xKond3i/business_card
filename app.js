...
function animate() {
  ...
  if (window.matchMedia("only screen and (max-width: 1024px)").matches) {
    ...
    tl.fromTo(greeting, 1, { y: 0 }, { y: window.innerHeight * 3, ease: Power3.easeInOut })
      ...
      .fromTo(content, 1, { margin: 0, height: 0, opacity: 0 }, { marginTop: 16, height: 'auto', opacity: 1, ease: Power3.easeInOut }, '+=.25');
  
  } else {
    // desktop animation
    tl.fromTo(greeting, 1, { x: 0 }, { x: window.innerWidth * 3, ease: Power3.easeInOut })
      .set(content, {transformOrigin:"50% 0%"})
      .fromTo(content, 1, { margin: 0, width: 0, opacity: 0 }, { marginLeft: 32, width: 'auto', opacity: 1, ease: Power3.easeInOut }, '+=.25');
  }
...
function handleStorage() {
  ...
  if (!theme) { localStorage['theme'] = 'light'; theme = 'light'; }
